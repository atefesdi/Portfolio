...
function Contact({}: Props) {
  ...
  const onSubmit: SubmitHandler<Inputs> = (data) => {
    if (data.email != "" && data.message !== "" && data.name !== "") {
      console.log(data);
      window.location.href = `mailto:atefe.saeedi1994@gmail?subject=newmessage&body=hi,${data.name} `
    }
  };
  ...
  return (
    <div className="h-screen  bg-[rgb(36,36,36)] relative flex flex-col justify-center items-center ">
      ...
      <form
        ...
        className="md:w-[650px] w-[350px] flex flex-col mt-10 gap-3"
      >
        <div className="flex flex-col md:flex-row w-full gap-3">
          <input
            ...
            type="text"
            placeholder="Name"
            className="contactInput h-[40px] w-full"
          />
          <input
            {...register("email")}
            type="email"
